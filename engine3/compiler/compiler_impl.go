...
func (c *CompilerImpl) Compile(ctx context.Context, args Args) (*engine.Spec, error) {
	...
	if stage == nil {
		return nil, fmt.Errorf("stage resource not found: %s", args.Stage.Name)
	}

	// extract the pipeline stage platform
	platform_ := new(harness.Platform)
	if v := stage.Platform; v != nil {
		platform_ = v
	}
	...
	if v := pipeline.Clone; v != nil {
		clone_ = v
	}
	if v := stage.Clone; v != nil {
	...
	envs := environ.Combine(
		...
		args.Build.Params,
		pipeline.Env, // environment from pipeline
		stage.Env,    // environment from stage
		environ.Proxy(),
	...
	for _, src := range stage.Steps {
		...
		if len(steps_) != 0 {
			for _, step_ := range steps_ {
				...
				if src.If != "" {
					onSuccess, onFailure, _ := evalif(src.If, inputs)
					switch {
	...
	for _, v := range stage.Volumes {
		...
		switch vv := v.With.(type) {
		case *harness.VolumeBind:
