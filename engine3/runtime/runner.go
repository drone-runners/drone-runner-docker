...
import (
	...
	"github.com/drone-runners/drone-runner-docker/engine3/compiler"
...
type Runner struct {
	...
	Resolver func(name, kind, typ, version string, id int64) (*harness.Schema, error)
...
func (s *Runner) run(ctx context.Context, stage *drone.Stage, data *client.Context) error {
	...
	lookupFunc := func(name string) (*harness.Template, error) {
		return nil, nil //s.Resolver(name, kind, typ, version, state.Build.ID)
	}
	...
	// extract the pipeline inputs and add to the expression contrext
	if vv := config.Pipeline.Inputs; vv != nil {
		inputParams["inputs"] = inputs.Inputs(vv, data.Build.Params)
	}
